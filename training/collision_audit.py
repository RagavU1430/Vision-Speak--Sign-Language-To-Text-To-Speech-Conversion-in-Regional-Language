...
CSV_PATH = os.path.join("dataset", "extracted_landmarks.csv")
...
class CollisionRuleEngine:
    ...
    # ── Fist-based (A = 0 fingers) ─────────────────────────────────────

    def _rule_count_aj(self, lm, _):
        fc = self.count_fingers(lm)
        return ("A", 0.90) if fc <= 1 else ("J", 0.85)

    def _rule_count_aw(self, lm, _):
        fc = self.count_fingers(lm)
        return ("A", 0.90) if fc <= 1 else ("W", 0.85)

    def _rule_count_ag(self, lm, _):
        fc = self.count_fingers(lm)
        return ("A", 0.85) if fc <= 1 else ("G", 0.85)

    # ── G (1 finger) vs R (2 crossed) ──────────────────────────────────

    def _rule_gr(self, lm, _):
        fc = self.count_fingers(lm)
        return ("G", 0.85) if fc == 1 else ("R", 0.85)

    # ── H (2 fingers) vs J (1 hook) ────────────────────────────────────

    def _rule_count_hj(self, lm, _):
        fc = self.count_fingers(lm)
        return ("J", 0.85) if fc <= 1 else ("H", 0.85)

    # ── I (pinky up) vs P (index+thumb circle) ─────────────────────────

    def _rule_ip(self, lm, _):
        n = self._extract_landmarks(lm)
    ...
    # ── P (index+thumb) vs Q (pinky+thumb) ─────────────────────────────

    def _rule_pq(self, lm, _):
        n = self._extract_landmarks(lm)
        thumb_idx = np.linalg.norm(n[4] - n[8])
        thumb_pin = np.linalg.norm(n[4] - n[20])
        return ("P", 0.80) if thumb_idx < thumb_pin else ("Q", 0.80)

    # ── S (fist) vs Z (index point) ────────────────────────────────────
    ...
    def _rule_count_gh(self, lm, _):
        fc = self.count_fingers(lm)
        return ("G", 0.95) if fc <= 1 else ("H", 0.90)
    ...
    def _rule_mn(self, lm, _):
        ...
        if extended >= 3:
            ...
            return "N", 0.85
        if np.linalg.norm(n[4] - n[5]) < 0.6:
            return "M", 0.70
        return None, 0.0

    def _rule_uv(self, lm, _):
        n = self._extract_landmarks(lm)
        d = np.linalg.norm(n[8] - n[12])
        return ("U", 0.90) if d < 0.3 else ("V", 0.90) if d >= 0.3 else (None, 0.0)

    def _rule_co(self, lm, _):
        d = self._tip_dist(lm, 4, 8)
        return ("O", 0.85) if d < 0.2 else ("C", 0.85) if d >= 0.3 else (None, 0.0)
    ...
    def _rule_st(self, lm, _):
        ...
        if fc == 0:
            return "S", 0.90
        n = self._extract_landmarks(lm)
        if np.linalg.norm(n[4] - n[5]) > 0.8:
            return "T", 0.80
        return None, 0.0

    def _rule_we(self, lm, _):
        fc = self.count_fingers(lm)
        return ("W", 0.85) if fc >= 3 else ("E", 0.80) if fc <= 1 else (None, 0.0)
    ...
    def verify(self, raw_prediction, second_prediction, probabilities, landmarks_21x3, class_names):
        ...
        rev_key = (second_letter, pred_letter)
